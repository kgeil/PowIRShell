...
function Get-clean_Ip {
  ...
  function Get-MicrosoftIPRanges {
    <#
    .SYNOPSIS
        Retrieves Microsoft's officially published Azure IP Ranges and Service
        Tags (Public Cloud), filtered to first-party M365 service tags, for use
    ...
    .DESCRIPTION
        ...
        function downloads Microsoft's published service-tag CIDR list so those
        IPs can be filtered out before spending threat-intel API budget on them
        or putting them in front of you in a gridview.

        CAVEAT: this only includes Microsoft's own *first-party* service
        endpoints (the -ServiceTags default), never the broader "AzureCloud"
        tag. AzureCloud also covers customer-leased VMs, and attackers routinely
        host infrastructure on Azure -- allowlisting all of AzureCloud would
        create a real blind spot. Only Office365 (and any other first-party
        tags you explicitly add) are trusted here.

        The download link Microsoft publishes for this file rotates (a new GUID
        each refresh), so this scrapes the current link off the download
        confirmation page rather than using a fixed URL. Results are cached
        locally so you're not re-downloading and re-scraping on every run; a
        cache older than -MaxCacheAgeDays is refreshed automatically, and a
        stale cache is used (with a warning) if the refresh fails, so a network
        hiccup mid-investigation doesn't dead-end you.
    .PARAMETER ServiceTags
        Which first-party Microsoft service tags to include. Default is
        @('Office365'), which itself covers Exchange Online, SharePoint Online,
        and Skype for Business/Teams service endpoints.
    .PARAMETER CachePath
    ...
    .EXAMPLE
        $msftRanges = Get-MicrosoftIPRanges -ServiceTags @('Office365','MicrosoftAzureActiveDirectory') -MaxCacheAgeDays 14
    #>
    [CmdletBinding()]
    param (
        [string[]]$ServiceTags = @('Office365'),
    ...
    if (-not $cacheIsFresh) {
        try {
            Write-Host "Fetching current Microsoft Azure IP Ranges and Service Tags list..." -ForegroundColor Yellow
            $confirmationPage = Invoke-WebRequest -Uri 'https://www.microsoft.com/en-us/download/confirmation.aspx?id=56519' -UseBasicParsing -ErrorAction Stop
            $jsonUrl = ($confirmationPage.Links | Where-Object { $_.href -match '^https://download\.microsoft\.com/download/.*ServiceTags_Public.*\.json$' } | Select-Object -First 1).href
            if (-not $jsonUrl) {
                # Fall back to scraping the raw HTML in case the parsed Links collection misses it
                $jsonUrl = [regex]::Match($confirmationPage.Content, 'https://download\.microsoft\.com/download/[^"]*ServiceTags_Public[^"]*\.json').Value
            }
            if (-not $jsonUrl) {
                throw "Could not locate the current ServiceTags_Public JSON download link on Microsoft's confirmation page -- the page layout may have changed."
            }
            $serviceTagData = Invoke-RestMethod -Uri $jsonUrl -ErrorAction Stop
            $serviceTagData | ConvertTo-Json -Depth 10 | Set-Content -Path $CachePath -Encoding UTF8 -Force
            Write-Host "Microsoft IP ranges refreshed and cached to $CachePath" -ForegroundColor Green
        } catch {
            Write-Host "Could not refresh Microsoft IP ranges: $($_.Exception.Message)" -ForegroundColor Yellow
            if (Test-Path $CachePath) {
                Write-Host "Falling back to cached copy from $((Get-Item $CachePath).LastWriteTime)" -ForegroundColor Yellow
                $serviceTagData = Get-Content $CachePath -Raw | ConvertFrom-Json
            } else {
    ...
    $cidrRanges = @()
    foreach ($tag in $serviceTagData.values) {
        if ($tag.name -in $ServiceTags) {
            $cidrRanges += ($tag.properties.addressPrefixes | Where-Object { $_ -notmatch ':' }) # IPv4 only
        }
    }
    $cidrRanges = $cidrRanges | Sort-Object -Unique
    Write-Host "Loaded $($cidrRanges.Count) Microsoft IPv4 CIDR ranges for allowlisting (tags: $($ServiceTags -join ', '))" -ForegroundColor Green
    return $cidrRanges
  }
